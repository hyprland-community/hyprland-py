    ...
    async def listen(self):
        async for event in socket2():
            match event[0]:
                case b"workspace":
                    if self._workspace:
                        asyncio.create_task(self._workspace(event[1].decode()))
                case b"focusedmon":
                    if self._focusedmon:
                ...
                case b"activewindowv2":
                    if self._activewindowv2:
                        asyncio.create_task(self._activewindowv2(event[1].decode()))
                case b"fullscreen":
                    if self._fullscreen:
                        asyncio.create_task(self._fullscreen(event[1] == b"1"))
                case b"monitorremoved":
                    if self._monitorremoved:
                        asyncio.create_task(self._monitorremoved(event[1].decode()))
                case b"monitoradded":
                    if self._monitoradded:
                        asyncio.create_task(self._monitoradded(event[1].decode()))
                case a:
                    msg = "Unhandled event. This is a bug, open an issue."
